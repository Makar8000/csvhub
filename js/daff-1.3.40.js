...
coopy.CompareFlags.prototype = {
	...
	,allowUpdate: function() {
		...
		return this.acts.exists("update") && this.acts.get("update");
	}
	,allowInsert: function() {
		if(this.acts == null) return true;
		return this.acts.exists("insert") && this.acts.get("insert");
	}
	,allowDelete: function() {
		if(this.acts == null) return true;
		return this.acts.exists("delete") && this.acts.get("delete");
	}
	,allowColumn: function() {
		if(this.acts == null) return true;
		return this.acts.exists("column") && this.acts.get("column");
	}
	...
	,getNameByRole: function(name,role) {
		...
		if(role == "parent") return parts[0];
		if(role == "local") return parts[parts.length - 2];
	...
	,getIdsByRole: function(role) {
		var result = [];
		if(this.ids == null) return result;
		var _g = 0;
		var _g1 = this.ids;
		while(_g < _g1.length) {
			var name = _g1[_g];
...
coopy.CompareTable.prototype = {
	...
	,alignCore: function(align) {
		if(this.useSql()) {
			...
			var sc = new coopy.SqlCompare(db,tab1,tab2,tab3,align,this.comp.compare_flags);
			sc.apply();
...
coopy.ConflictInfo = function(row,col,pvalue,lvalue,rvalue) {
	this.row = row;
	this.col = col;
	this.pvalue = pvalue;
	this.lvalue = lvalue;
...
coopy.ConflictInfo.__name__ = true;
coopy.ConflictInfo.prototype = {
	__class__: coopy.ConflictInfo
};
...
coopy.Coopy.keepAround = function() {
	...
	var sc = new coopy.SqlCompare(null,null,null,null,null);
...
coopy.Coopy.prototype = {
	...
	,encodeTable: function(name,t,render) {
		...
		if(render == null) {
			if(this.format_preference == "csv") {
				var csv = new coopy.Csv(this.delim_preference,this.csv_eol_preference);
				txt = csv.renderTable(t);
			} else if(this.format_preference == "ndjson") txt = new coopy.Ndjson(t).render(); else if(this.format_preference == "html" || this.format_preference == "www") {
	...
	,saveTables: function(name,os,use_color,is_diff) {
		...
		this.checkFormat(name);
		var render = null;
		if(use_color) render = new coopy.TerminalDiffRender(this.flags,this.delim_preference,is_diff);
	...
	,runDiff: function(parent,a,b,flags,output) {
		...
		if(this.fail_if_diff) {
			...
			if(summary.different) this.diffs_found = true;
		}
	}
	,loadTable: function(name,role) {
		...
		if(ext == "sqlite") {
			...
			var tab = new coopy.SqlTables(sql,this.flags,role);
			return tab;
		}
		...
		if(this.csv_eol_preference == null) this.csv_eol_preference = csv.getDiscoveredEol();
		if(output != null) output.trimBlank();
		return output;
	}
	...
	,run: function(args,io) {
		...
		while(more) {
			...
			while(_g1 < _g) {
				...
				if(tag == "--output") {
					...
					if(context1 >= 0) this.flags.unchanged_column_context = context1;
					args.splice(i,2);
					break;
				} else if(tag == "--inplace") {
					more = true;
					...
					more = true;
					var ending = args[i + 1];
					if(ending == "crlf") ending = "\r\n"; else if(ending == "lf") ending = "\n"; else if(ending == "cr") ending = "\r"; else if(ending == "auto") ending = null; else {
		...
		try {
			...
			if(args.length - offset >= 2) {
				if(cmd1 != "copy") b = this.loadTable(args[1 + offset],"remote"); else output = args[1 + offset];
			}
			...
			if(cmd1 == "diff") {
				...
				if(this.fail_if_diff) {
					...
					if(this.diffs_found) return 1;
				} else this.runDiff(parent,a,b,this.flags,output);
			} else if(cmd1 == "patch") {
				...
				this.saveTable(output,a);
			} else if(cmd1 == "trim") this.saveTable(output,a); else if(cmd1 == "render") this.renderTable(output,a); else if(cmd1 == "copy") {
				var os = new coopy.Tables(a);
				os.add("untitled");
				this.saveTables(output,os,this.useColor(this.flags,output),false);
			}
		} catch( e1 ) {
			if(!this.fail_if_diff) throw e1;
			return 2;
		}
		if(ok) return 0; else if(this.fail_if_diff) return 2; else return 1;
	}
	,coopyhx: function(io) {
		var args = io.args();
...
coopy.Csv = $hx_exports.coopy.Csv = function(delim,eol) {
	if(delim == null) delim = ",";
	this.cursor = 0;
...
coopy.Csv.prototype = {
	...
	,renderCell: function(v,d,force_quote) {
		if(force_quote == null) force_quote = false;
		if(d == null) return "NULL";
		var str = v.toString(d);
		var need_quote = force_quote;
		if(!need_quote) {
			if(str.length > 0) {
				if(str.charAt(0) == " " || str.charAt(str.length - 1) == " ") need_quote = true;
			}
		}
		if(!need_quote) {
			var _g1 = 0;
		...
		if(line_buf.length > 0) result += line_buf;
		if(need_quote) result += "\"";
		return result;
	}
				...
				if(!quoting) {
					...
					if(ch == 13 || ch == 10) {
						...
						if(ch2 != null) {
							if(ch2 != ch) {
								if(ch2 == 13 || ch2 == 10) {
									if(this.discovered_eol == null) this.discovered_eol = String.fromCharCode(ch) + String.fromCharCode(ch2);
									last_processed++;
								}
							}
						}
						if(this.discovered_eol == null) this.discovered_eol = String.fromCharCode(ch);
						this.row_ended = true;
						break;
					}
					if(ch == 34) {
	...
	,render: function(tab) {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				...
				var val;
				if(this.pretty_arrows) val = cell.pretty_value; else val = cell.value;
				if(this.quote_html) val = StringTools.htmlEscape(view.toString(val));
...
coopy.DiffSummary = $hx_exports.coopy.DiffSummary = function() {
};
coopy.DiffSummary.__name__ = true;
coopy.DiffSummary.prototype = {
	__class__: coopy.DiffSummary
};
...
coopy.Merger.prototype = {
	...
	,addConflictInfo: function(row,col,view,pcell,lcell,rcell) {
		this.conflict_infos.push(new coopy.ConflictInfo(row,col,view.toString(pcell),view.toString(lcell),view.toString(rcell)));
	}
...
coopy.SqlCompare = $hx_exports.coopy.SqlCompare = function(db,local,remote,alt,align,flags) {
	this.db = db;
	this.local = local;
	this.remote = remote;
	this.alt = alt;
	this.align = align;
	this.flags = flags;
	if(this.flags == null) this.flags = new coopy.CompareFlags();
	this.peered = false;
...
coopy.SqlCompare.prototype = {
	...
	,validateSchema: function() {
		...
		if(this.local != null) {
			...
			if(this.flags.ids != null) key_cols1 = this.flags.getIdsByRole("local");
		...
		if(this.remote != null) {
			...
			if(this.flags.ids != null) key_cols2 = this.flags.getIdsByRole("remote");
		...
		if(this.alt != null) {
			...
			if(this.flags.ids != null) key_cols3 = this.flags.getIdsByRole("parent");
		...
		if(pk_change) throw "sql diff not possible when primary key changes: " + Std.string([key_cols1,key_cols2,key_cols3]);
		return true;
	}
	,denull: function(x) {
		if(x == null) return -1;
	...
	,apply: function() {
		...
		if(this.local != null) {
			...
			if(this.flags.ids != null) {
				...
				data_cols = [];
				var pks = new haxe.ds.StringMap();
				var _g = 0;
				while(_g < key_cols.length) {
					var col = key_cols[_g];
					++_g;
					if(__map_reserved[col] != null) pks.setReserved(col,true); else pks.h[col] = true;
				}
				var _g1 = 0;
				while(_g1 < all_cols.length) {
					var col1 = all_cols[_g1];
					++_g1;
					if(!(__map_reserved[col1] != null?pks.existsReserved(col1):pks.h.hasOwnProperty(col1))) data_cols.push(col1);
				}
			}
		}
		if(this.remote != null) {
		...
		var sql_all_cols = "";
		var _g18 = 0;
		var _g9 = all_common_cols.length;
		while(_g18 < _g9) {
			var i7 = _g18++;
		...
		var _g116 = 0;
		var _g26 = data_common_cols.length;
		while(_g116 < _g26) {
			var i15 = _g116++;
			if(i15 > 0) sql_data_mismatch += " OR ";
			var n4 = common.getQuotedColumnName(data_common_cols[i15]);
			sql_data_mismatch += sql_table1 + "." + n4 + " IS NOT " + sql_table2 + "." + n4;
		}
		var _g117 = 0;
		var _g27 = all_cols2.length;
		while(_g117 < _g27) {
			var i16 = _g117++;
			var key5 = all_cols2[i16];
			if(!(__map_reserved[key5] != null?present1.existsReserved(key5):present1.h.hasOwnProperty(key5))) {
				...
				var n5 = common.getQuotedColumnName(key5);
				sql_data_mismatch += sql_table2 + "." + n5 + " IS NOT NULL";
			}
		}
		if(this.alt != null) {
			var _g118 = 0;
			var _g28 = data_common_cols.length;
			while(_g118 < _g28) {
				var i17 = _g118++;
...
coopy.SqlTables = $hx_exports.coopy.SqlTables = function(db,flags,role) {
	this.db = db;
	...
	if(flags.tables != null) {
		...
		while(_g < _g1.length) {
			...
			if(__map_reserved[key] != null) allowed.setReserved(key,value); else allowed.h[key] = value;
		}
		count = 0;
		var _g2 = 0;
		while(_g2 < names.length) {
...
coopy.TableDiff.prototype = {
	...
	,reset: function() {
		...
		this.col_renames = 0;
		this.col_reorders = 0;
		this.column_units_updated = new haxe.ds.IntMap();
	}
		...
		while(_g1 < _g) {
			...
			if(cunit.r >= 0 && cunit.lp() == -1) {
				...
				if(this.active_column != null) {
					if(this.allow_column) this.active_column[j] = 1;
				}
				if(this.allow_column) this.col_inserts++;
			}
			if(cunit.r < 0 && cunit.lp() >= 0) {
				...
				if(this.allow_column) this.col_deletes++;
			}
			if(cunit.r >= 0 && cunit.lp() >= 0) {
	...
	,checkMeta: function(t,meta) {
		...
		if(t == null) return meta.get_width() == 1 && meta.get_height() == 1;
	...
	,getMetaTable: function(t) {
		if(t == null) {
			var result = new coopy.SimpleTable(1,1);
			result.setCell(0,0,"@");
			return result;
		}
		var meta = t.getMeta();
	...
	,addMeta: function(output) {
		if(this.a == null && this.b == null && this.p == null) return false;
				...
				if((have_pp?cunit.p:cunit.l) < 0) {
					if(rr != null) {
						if(this.v.toString(rr) != "") {
							if(this.allow_column) this.have_addition = true;
						}
					}
				}
			}
...
coopy.TerminalDiffRender = $hx_exports.coopy.TerminalDiffRender = function(flags,delim,diff) {
	if(diff == null) diff = true;
	...
	if(delim != null) this.delim = delim; else this.delim = ",";
	this.diff = diff;
};
...
coopy.TerminalDiffRender.prototype = {
	...
	,render: function(t) {
		...
		while(_g < h) {
			...
			while(_g1 < w) {
				...
				if(x > 0) {
					txts.push(this.codes.get("minor"));
					txts.push(this.delim);
					txts.push(this.codes.get("done"));
				}
	...
	,getText: function(x,y,color) {
		...
		if(color && this.diff) {
			...
			if(code != null) {
				var separator;
				if(this.use_glyphs) separator = cell.pretty_separator; else separator = cell.separator;
				if(cell.rvalue != null) {
					val = this.codes.get("remove") + cell.lvalue + this.codes.get("modify") + separator + this.codes.get("add") + cell.rvalue + this.codes.get("done");
					if(cell.pvalue != null) val = this.codes.get("conflict") + cell.pvalue + this.codes.get("modify") + separator + Std.string(val);
				} else {
					if(this.use_glyphs) val = cell.pretty_value; else val = cell.value;
					val = code + Std.string(val) + this.codes.get("done");
				}
			}
		} else if(color && !this.diff) {
			if(y == 0) val = this.codes.get("header") + Std.string(val) + this.codes.get("done");
		} else if(this.use_glyphs) val = cell.pretty_value; else val = cell.value;
	...
	,pickSizes: function(t) {
		...
		while(_g < w) {
			...
			while(_g1 < h) {
				...
				if(txt == "@@" && row == -1 && this.diff) row = y;
				if(row == -1 && !this.diff) row = y;
				var len = txt.length;
				if(y == row) mmin = len;
				m += len;
				m2 += len * len;
		...
		if(total > 130 && !this.wide_columns) return null;
		return sizes;
	}
